...
static int handle_scan_mode(const std::string& source_dir, const std::string& output_dir) {
    ...
    if (!fs::exists(source_path) || !fs::is_directory(source_path)) {
        std::cerr << "Error: Source directory ('" << source_dir << "') does not exist!\n";
        return 1;
    }
    ...
    std::cout << "[3/3] Saving records to SQLite database...\n";
    Database db(full_db_path.string());
    if (!db.open() || !db.initialize_schema() || !db.save_records(records)) {
        std::cerr << "Database error: " << db.last_error() << "\n";
        return 1;
    }

    std::cout << "\nSuccessfully saved:\n-> " << full_db_path.string() << " (" << records.size() << " files)\n";
    return 0;
}
...
int main(int argc, char* argv[]) {
    ...
    if (!opts.error_message.empty()) {
        ...
        std::cerr << "Tip: Use 'backup_integrity_checker.exe -h' for help.\n";
        return 1; 
    }
